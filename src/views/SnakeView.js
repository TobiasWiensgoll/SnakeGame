export default class SnakeView {
  constructor(scene, snakeModel) {
    ...
    // this.snakeHead = this.scene.physics.add.sprite(300, 10000, "snakeUp");
    // this.model.body.push(this.snakeHead);
    // this.tail = this.scene.physics.add.sprite(
    //   this.scene.scale.width / 2 - this.model.field.tileSize,
  ...
  setBodyPartTexture(i, oldBodyDirection) {
    if (!oldBodyDirection.equals(this.model.directions[i - 1])) {
      let prevDirection = `${this.model.directions[i - 1].x},${
        this.model.directions[i - 1].y
      }`;
